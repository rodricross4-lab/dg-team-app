...
export default function CloudStudentProfilePanel({ studentId, studentName }: Props) {
  const {
    ...
    insights,
    loadProfile
  } = useCloudStudentProfile(studentId);
  ...
  return (
    <div style={panel}>
      <div style={header}>
        ...
        <button style={button} onClick={loadProfile} disabled={loading}>
      ...
      <div style={grid}>
        <div style={card}><strong>{workouts.length}</strong><span>Treinos</span></div>
        <div style={card}><strong>{sessions.length}</strong><span>Sessões</span></div>
        <div style={card}><strong>{logbook.length}</strong><span>Logbook</span></div>
        <div style={card}><strong>{assessments.length}</strong><span>Avaliações</span></div>
        <div style={card}><strong>{checkins.length}</strong><span>Check-ins</span></div>
        <div style={card}><strong>{timeline.length}</strong><span>Timeline</span></div>
        <div style={card}><strong>{insights.length}</strong><span>Insights IA</span></div>
      </div>
    </div>
  );
}
...
const hero = { background: '#180909', border: '1px solid #351111', borderRadius: 18, padding: 16, marginBottom: 16 };
...
const card = { background: '#0b0b0b', border: '1px solid #1f1f1f', borderRadius: 16, padding: 16, display: 'grid', gap: 8, textAlign: 'center' as const, color: '#fff' };
